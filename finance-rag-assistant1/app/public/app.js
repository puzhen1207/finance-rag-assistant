...
async function refreshHistory() {
  ...
  $("historyCount").textContent = `${history.length} logs`;
  $("historyList").innerHTML = history.length
    ? history.map((item) => `
      <article class="history-item">
        <div>
          <b>${escapeHtml(item.question)}</b>
          <span>${item.found ? "命中" : "拒答"} · ${item.source_count} sources</span>
        </div>
        <p>${escapeHtml(item.answer).slice(0, 180)}${item.answer.length > 180 ? "..." : ""}</p>
        <small>${formatTime(item.created_at)}${item.top_score !== null && item.top_score !== undefined ? ` · top ${item.top_score}` : ""}</small>
      </article>
    `).join("")
...
function renderSources(sources) {
  ...
  sourcesList.innerHTML = sources.length
    ...
    `).join("")
    : `<div class="empty-note">没有可展示的来源。</div>`;
}
