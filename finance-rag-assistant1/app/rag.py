...
def build_extractive_answer(question: str, sources: list[Source]) -> str:
    ...
    for source in sources:
        ...
        for part in parts:
            ...
            if len(cleaned) < 12:
                continue
            terms = {t.lower() for t in re.findall(r"[\w\u4e00-\u9fff]+", cleaned) if len(t) > 1}
            scored.append((len(question_terms & terms), len(cleaned), cleaned))
        for _, _, passage in sorted(scored, reverse=True)[:2]:
    ...
    for i, passage in enumerate(passages[:5], start=1):
        cited.append(f"{i}. {passage}")
    return "根据已检索到的知识库片段，可以整理如下：\n" + "\n".join(cited)
